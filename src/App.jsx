...
import { Wine, Star, Trash2, X, Plus, Filter, ChevronDown, ChevronUp, Search, Bookmark, Heart, LogOut, Check } from 'lucide-react';

export default function WineJournal() {
  const [wines, setWines] = useState([
    { id: 1, nombre: 'Saint Felicien', bodega: 'Catena Zapata', cosecha: '2023', uva: 'Pinot Noir', region: 'Mendoza', subregion: 'Luján de Cuyo', probado: true, rating: 4, comentario: 'Rico, no falla' },
    { id: 2, nombre: 'Ver Sacrum', bodega: 'Ver Sacrum', cosecha: '2019', uva: 'Monastrell', region: 'Mendoza', subregion: 'Valle de Uco', probado: true, rating: 5, comentario: 'Riquísimo' },
    { id: 3, nombre: 'Enemigo', bodega: 'El Enemigo', cosecha: '2020', uva: 'Malbec', region: 'Mendoza', subregion: '', probado: false, rating: 0, comentario: '' },
  ]);
  ...
  const saveWine = (probado) => {
    if (!isFormValid) return;
    const wine = { ...form, id: Date.now(), probado, rating: 0, comentario: '' };
    if (probado) {
      setPendingWine(wine);
      setShowRating(true);
    } else {
  ...
  const confirmRating = (rating) => {
    if (pendingWine) {
      const existingWine = wines.find(w => w.id === pendingWine.id);
      if (existingWine) {
        setWines(wines.map(w => w.id === pendingWine.id ? { ...pendingWine, rating, comentario } : w));
      } else {
        setWines([...wines, { ...pendingWine, rating, comentario }]);
        setForm({ nombre: '', bodega: '', cosecha: '', uva: '', region: '', subregion: '' });
      }
    }
    setShowRating(false);
  ...
  const deleteWine = (id) => setWines(wines.filter(w => w.id !== id));

  const moveWine = (id) => {
    const wine = wines.find(w => w.id === id);
    if (!wine) return;
    if (!wine.probado) {
      setPendingWine({ ...wine, probado: true });
      setShowRating(true);
    } else {
      setWines(wines.map(w => w.id === id ? { ...w, probado: false, rating: 0, comentario: '' } : w));
    }
  };
      ...
      <header style={{ height: '75px', display: 'flex', alignItems: 'center' }}>
        <div className="max-w-md mx-auto w-full px-4">
          <div className="flex items-center justify-between">
            ...
            <button className="p-2 rounded-full" style={{ background: '#722F37' }}>
